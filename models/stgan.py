...
class Generator(nn.Module):
    ...
    def forward(self, recent_data, trend_data, graph, time_feature):
        """Generator
        :param recent_data: (B, T_recent, V, L*D)
        :param trend_data: (B, T_trend, V, L*D)
        :param graph: (B, V, V)
        :param time_feature: (B, d)
        :return
        - Output: `2-D` tensor with shape `(B, L*D)`
        """
        batch_size = recent_data.shape[0]
        recent, _ = self.recent_network(recent_data, graph)  # (B, num_adj, rnn_units)
        
        trend_data = trend_data.reshape((trend_data.shape[0]*trend_data.shape[2], trend_data.shape[1],-1))
        trend, _ = self.trend_network(trend_data)  # (B*V, seq_len, hidden_dim)
        trend = trend[:, -1, ].view(recent_data.shape[0], recent_data.shape[2], -1)  # (B*V, hidden_dim) --> (B, V, hidden_dim)
        ...
        output = self.gcn(combined, graph)
